...
export default function CandidateLifecycleTab() {
  ...
  const run = async (id, action) => { setBusy(id); setError(""); setNotice(""); try { const result = await action(); if (result.data?.invitationToken) { const link = result.data.invitationUrl || `${window.location.origin}/candidate/offer/${result.data.invitationToken}`; let copied = false; try { await navigator.clipboard?.writeText(link); copied = Boolean(navigator.clipboard); } catch { copied = false; } const delivery = result.data.emailDelivery; if (delivery?.status === "sent") setNotice(`${delivery.message} ${copied ? "The invitation link was also copied." : `Manual link: ${link}`}`); else setNotice(`${delivery?.message || "Invitation generated."} ${copied ? "The invitation link was copied." : `Manual link: ${link}`}`); } else if (result.data?.temporaryPassword) { setNotice(`Employee created. Login: ${result.data.loginEmail} | Temporary password: ${result.data.temporaryPassword}`); } else setNotice("Workflow updated successfully."); await load(); } catch (e) { setError(e.message); } finally { setBusy(""); } };
